...
- **Shadow DOM:** the pin's own shadow-DOM isolation comes from global `pinShadowDOM` (default `false`), **not** from a per-pin prop — `VeltCommentPin` exposes no `shadowDom` prop. (Confirmed: the pin **marker** defaults `pinShadowDOM: false`, while the pin **dialog** defaults `dialogShadowDom: true` — the two surfaces isolate independently; see [`../behaviors.md`](../behaviors.md).)
...
Counted against [`props.md`](../props.md) `### VeltCommentBubble` / `### VeltCommentPin` / `### VeltCommentTool`:
