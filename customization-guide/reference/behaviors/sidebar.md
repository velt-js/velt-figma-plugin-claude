# Behaviors · Comments Sidebar

Runtime behavior of every prop on the three sidebar entry points. [`props.md`](../props.md) gives each prop's **name, type, and default**; this file gives what each prop **does at runtime**, its **default behavior** when omitted, and how it **interacts** with related props. See the [behaviors index](../behaviors.md) for the dialog/composer state machine and the cross-component prop matrix.
...
| `openSidebar` | — | **Deprecated alias of `onSidebarOpen`** — emits when the sidebar opens. | Use `onSidebarOpen`; both fire. |
| `onSidebarOpen` | — | Callback fired when the sidebar opens. | — |
| `onSidebarCommentClick` | — | **Deprecated alias of `onCommentClick`** — emits the clicked comment. | Use `onCommentClick`. |
| `onCommentClick` | — | Callback fired when a comment row is clicked (payload is the comment/annotation). | With `urlNavigation` on, the click also drives URL/selection. |
| `onSidebarClose` | — | Callback fired when the sidebar closes. | — |
| `onCommentNavigationButtonClick` | — | Callback for the prev/next thread navigation buttons inside the sidebar. | — |
...
| `fullExpanded` | `false` | Forces thread cards to render **fully expanded** (all comments) rather than collapsed previews. | Overrides the default collapsed-preview behavior described in the [behaviors index](../behaviors.md) collapse matrix. |
...
| `commentPlaceholder` | SDK default | Placeholder text for the **new-comment** composer input. | — |
| `replyPlaceholder` | SDK default | Placeholder text for the **reply** composer input. | — |
...
| `queryParamsComments` | `false` | Query-param comment selection. | — |
...
| `dialogVariant` | `'sidebar'` | Wireframe variant for dialogs opened in the sidebar. | — |
...
| `searchPlaceholder` | `'Search comments'` | Placeholder for the search input. | — |
| `commentPlaceholder` | `''` | New-comment composer placeholder. | — |
| `replyPlaceholder` | `''` | Reply composer placeholder. | — |
...
| `onSidebarOpen` | — | Sidebar-opened callback. | — |
| `openSidebar` | — | **Deprecated alias of `onSidebarOpen`.** | Use `onSidebarOpen`. |
| `onSidebarClose` | — | Sidebar-closed callback. | — |
| `onCommentClick` | — | Comment-row-clicked callback. | — |
| `onSidebarCommentClick` | — | **Deprecated alias of `onCommentClick`.** | Use `onCommentClick`. |
| `onCommentNavigationButtonClick` | — | Prev/next thread navigation callback. | — |
| `onFullscreenClick` | — | Fired when the fullscreen toggle is clicked. **V2-only** event (no V1 equivalent on the public surface). | Pairs with `fullScreen`. |
...
| `urlNavigation` | `null` | **Pass-through** URL navigation for the opened sidebar. | — |
| `currentLocationSuffix` | `null` | **Pass-through:** append current location. | — |
| `filterConfig` | `null` | **Pass-through:** which filter dimensions appear in the opened sidebar. | Same shape as V1 `filterConfig`. |
| `groupConfig` | `null` | **Pass-through:** grouping config for the opened sidebar. | — |
| `filters` | `null` | **Pass-through:** active filter selections for the opened sidebar. | — |
...
| `dialogVariant` | `'sidebar'` | **Pass-through:** dialog wireframe variant for the opened sidebar. | — |
...
| `position` | `'right'` | **Pass-through:** which side the opened sidebar anchors to. | — |
| `filterPanelLayout` | `'bottomSheet'` | **Pass-through:** filter panel layout for the opened sidebar. | — |
...
| `onCommentClick` | — | Fires when a comment is clicked in the opened sidebar. | — |
| `onSidebarOpen` | — | Fires when the sidebar opens (button toggled it on). | — |
...
No items are marked unconfirmed — every behavior above is verified against component source.
