...
- **Compounding** — two layers paint the *same* property (owner + a wrapper both have padding → the M1 bug: `velt-sidebar-container` + `app-comment-sidebar-list` + your `.hw-*` each adding padding). Keep the load-bearing one, **`neutralize[]`** the duplicates (`padding:0`/… `!important`).
...
- [ ] Verified by **whole-surface measurement** (style + layout + visual gate) vs the `designSpec`, auto-derived from every mapped element, no aggregate; "looks close"/sampling rejected; termination is a separate `/goal` evaluator (R20).
...
- [ ] Termination is mechanical — `judge-report.json` covers 100% of the **generated** `checklist.json` (no sampling) + the visual artifact per state; `verdict-gate.mjs` returns PASS (INCOMPLETE ≠ done) (R26).
