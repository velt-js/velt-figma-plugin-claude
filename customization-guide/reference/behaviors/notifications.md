...
Runtime behavior of every prop on the two notification entry points. [`props.md`](../props.md) gives each prop's **name, type, and default**; this file gives what each prop **does at runtime**, its **default** when omitted, and how it **interacts** with related props. See the [behaviors index](../behaviors.md) for the cross-component prop matrix and the anchored-vs-static positioning model.
...
| `onNotificationClick` | — | Callback fired when a notification row in the **opened panel** is clicked; receives the `Notification` (wrapper passes `event.detail`). | The tool re-emits the panel's click as its own `onNotificationClick` event. Internal click handling (read/navigate) still runs. |
...
| `enableCrossOrganization` | off | Cross-org notifications; same `true`/string/`CrossOrganizationConfig` handling as the panel (object `enabled: false` disables). | **Global via the service.** Same array-falls-through-to-plain-enable note as the panel (`(unconfirmed)` whether intended). |
...
Unconfirmed items (explicitly marked above): the phone bottom-sheet breakpoint for `panelOpenMode`; whether the array-binding fall-through on `enableCrossOrganization` is intentional. Every other behavior is verified against component, service, and config-model source.
