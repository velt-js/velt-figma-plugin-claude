...
2. **Reuse one browser/page across a block's iterations.** Fast Refresh re-renders on edit; relaunching Chromium + `networkidle` + re-seeding every iteration is wasteful. Keep the page alive, reset+re-assert state each iteration (the existing Escape/clear/blur recipe), re-seed only if the doc changed. Add `capture-block.mjs --reuse` / a persistent-page mode.
...
- **`strictly wireframe`** — decision-tree may assign only default/prop/CSS/wireframe; primitives/headless forbidden → declare GAP instead of escalating. Oracle set unchanged.
