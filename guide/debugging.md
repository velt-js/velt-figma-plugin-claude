...
- **Fix B (request interception) — NOW BUILT IN, no action needed.** `scripts/_browser-env.mjs` is the single chromium loader for every browser-driving script, and it installs a `route()` interceptor that performs each external request from Node through `$HTTPS_PROXY` and fulfills the page with the response. It self-arms only when an egress proxy is configured (`VELT_EGRESS_SHIM=auto`, the default), so it is inert on an ordinary machine. Force it with `VELT_EGRESS_SHIM=on`, disable with `=off`, and trace it with `VELT_EGRESS_DEBUG=1`. Confirm the environment with `node scripts/_browser-env.mjs` (prints proxy, shim state, CA bundle, playwright-core, and a live `cdn.velt.dev` probe). Caveat: `route()` cannot intercept **WebSockets**, so `wss://…firebaseio.com` still resets — harmless, because Velt's realtime stack falls back to HTTPS long-polling, which IS intercepted (verified end-to-end: comments read *and* write).
- **Fix A (self-provisioned local proxy)** remains the fallback if a page genuinely needs WebSockets.
