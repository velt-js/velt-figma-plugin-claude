# Reference · Component definitions

A flat list of every Velt component — top-level components and every sub-component / wireframe slot, small to small — with a short definition of what each one does. Use it to look up what any component is, or to figure out which component a design element corresponds to.
...
Names are the wireframe custom-element tags (e.g. `velt-comment-dialog-header-wireframe`); the React primitive (e.g. `VeltCommentDialog`) is noted in the definition for top-level components. Every name is from the SDK source — if it isn't here, it doesn't exist.
...
- `velt-comment-dialog-wireframe` — React: `VeltCommentDialog`. The main popover/card that appears when a comment pin is opened, containing the full thread of comments plus header, body, composer, and action controls. It is where users read a conversation and reply, resolve, assign, or manage it.
...
- `velt-comment-dialog-status-dropdown-wireframe` — The dropdown control wrapping the status selector for the thread. It lets the user change the thread's workflow status.
...
- `velt-comment-dialog-priority-dropdown-wireframe` — The dropdown control wrapping the priority selector. It lets the user set or change the thread's priority.
...
- `velt-comment-thread-wireframe` — React: `VeltCommentThread`. A standalone rendering of a single comment thread outside the pin dialog (e.g. embedded in custom UI). It shows the conversation and composer for one thread.
...
- `velt-comments-sidebar-panel-wireframe` — React: `VeltCommentsSidebar`. The main sidebar panel that lists all comment threads for the document. It is where users browse, filter, and open comments in one place.
...
- `velt-comments-sidebar-filter-button-wireframe` — The button that opens the sidebar filter controls. It lets the user start filtering the comment list.
...
- `velt-comments-sidebar-minimal-filter-dropdown-wireframe` — A compact filter dropdown offering common preset filters and sorts. It gives users quick one-click filtering in minimal mode.
...
- `velt-comments-sidebar-v2-wireframe` — React: `VeltCommentsSidebarV2`. The next-generation comments sidebar with an updated layout and grouping. It is the modern panel for browsing and managing all comments.
...
- `velt-comments-sidebar-filter-container-v2-wireframe` — The full V2 filter container panel with sections and controls. It is the comprehensive filtering surface in V2.
...
- `velt-inline-comments-section-wireframe` — React: `VeltInlineCommentsSection`. An embedded comments section that lives inline within page content (not a floating dialog). It shows a thread list and composer in place for a specific location.
...
- `velt-multi-thread-comment-dialog-wireframe` — React: `VeltMultiThreadCommentDialog`. A dialog that displays multiple comment threads together in one popover (e.g. all comments at a shared location). It lets the user browse and manage several threads at once.
