...
| **Renamed tag** | A selector built from the manifest tag matches nothing, though the primitive is on screen. | Build selectors from `emitsTag`. For primitives that now equals the manifest tag — `VeltCommentDialogComposerInput` emits `velt-comment-dialog-composer-input`. The previous "366 of 441 render a `-wireframe` element" was a `sync-primitives.mjs` collision bug (two wrappers share each reactName; the tiebreak always picked the wireframe twin), which also mis-recorded `forwardsClassName:true` for 363 wrappers that drop `className`. Both fixed — but keep reading `emitsTag` rather than assuming. |
