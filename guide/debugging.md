...
- **Fix B (request interception) — now BUILT IN, no action needed:** `scripts/lib/browser-egress.mjs` intercepts every external request the page makes (`route()`), performs it from Node (which the proxy tolerates), and fulfills the page with the response. Every measurement script that opens a browser is wired to it, so `launch()`, `connect()` and `connectOverCDP()` all come back already armed. It switches on **automatically** when an egress proxy is configured (`HTTPS_PROXY`/`HTTP_PROXY`); force it with `VELT_BROWSER_PROXY_FETCH=1`, disable it with `=0` (e.g. when you are using Fix A instead). Limits: loopback is never intercepted (the app under test is untouched), responses are buffered rather than streamed, and **WebSockets cannot be intercepted** — Velt's Firebase transport falls back to HTTP long-polling on its own, which IS intercepted, so the sidebar still renders real data.
...
### "playwright-core not found" even though Playwright is installed
The loaders try `$PLAYWRIGHT_CORE`, then a **bare** `playwright-core` specifier — which only resolves for a *local* install. A **globally** installed Playwright (`npm i -g playwright`, the shape most cloud runners ship) sits outside Node's resolution path entirely. `globalPlaywrightCore()` in `scripts/lib/browser-egress.mjs` now derives that global location from `process.execPath` and every loader falls back to it, so a global install works with no `PLAYWRIGHT_CORE` export. If you still hit this, set `PLAYWRIGHT_CORE=/abs/path/to/playwright-core/index.js` explicitly.
