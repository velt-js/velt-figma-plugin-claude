...
> **This page is the index; defaults + behavior live in [`behaviors.md`](./behaviors.md) (and `behaviors/`) — the single source of truth for what a prop *does*, its **default**, and how props combine.** Look a prop up here to find its name/type, then read its real default + runtime behavior + interactions there. For data fields/events behind a prop → [`data-models.md`](./data-models.md); off‑vs‑on by default → [`feature-flags.md`](./feature-flags.md); layout/mode + custom‑data shapes → [`component-config.md`](./component-config.md); also [`apis.md`](./apis.md) and [`events.md`](./events.md).
