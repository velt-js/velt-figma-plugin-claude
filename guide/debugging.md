...
- **Fix B (request interception) — shipped as `scripts/sandbox-egress.mjs`, opt in with `VELT_SANDBOX_EGRESS=1`:** every request the page makes is intercepted and re-pointed at a local Node HTTPS server that performs the real request through the sandbox proxy and **streams** the response back; WebSockets go through `routeWebSocket` + `connectToServer`. `measure-block.mjs`, `verify-app.mjs`, `capture-block.mjs` and `browser-endpoint.mjs --launch` already call it, so no per-script wiring is needed.
  - Note `route.fulfill()` alone is NOT enough: it needs a complete body, so it cannot represent Firestore's WebChannel backchannel (a long-lived GET). Buffering it desynchronises the session, Google answers 400, and the comments sidebar renders its shell and stays empty — which reads like a customization bug, not an egress one.
