...
| History panel (inline) | `VeltNotificationsHistoryPanel` | — |

## Config props (cheapest customization — do this first)

On `VeltNotificationsTool` / `VeltNotificationsPanel`:
...
| `darkMode` / `shadowDom` / `variant` | — | theme / shadow‑DOM / variant; Tool also has `panelShadowDom` / `panelVariant` |
