# Reference · Wireframe variables (per component, with usage)

Every `{…}` token you can read inside a wireframe, **organized by component**, with an **example usage** for each. For the token *syntax* (operators, nesting) see [`wireframe-tokens.md`](./wireframe-tokens.md).
...
> Generated from the SDK and exhaustive per component. If a name isn't listed for a component, it isn't available there. For the nested‑access root list and operators, see [`wireframe-tokens.md`](./wireframe-tokens.md).
...
| `repliesUniqueUsers` | array | app | distinct reply authors | — | |
...
| `assignTo` / `selectedUserContacts` | object/array | data | assign‑menu selection | — | |
| `customList` / `taggedUserContacts` / `taggedGroups` | array | data | autocomplete / mentions | — | |
| `selectedFiles` / `invalidSelectedFiles` / `selectedAttachments` / `attachmentsToDelete` | array | data | composer files | — | |
| `localRecordedData` | array | data | staged recordings | — | |
...
| `composerContent` / `composerContentHTML` (alias `newComment`/`newCommentHTML`) | string | ui | draft text / html | — | |
| `showReplies` / `collapsedComments` | bool | ui | replies shown / collapsed | — | 🔑 |
| `darkMode` / `readOnly` | bool | ui | theme / read‑only | — | 🔑 |
| `selectedVisibility` / `selectedVisibilityUsers` | string/array | ui | visibility selection | — | |
| `isFocusedThreadEnabled` | bool | ui | focused‑thread allowed | `veltIf="{isFocusedThreadEnabled}"` | 🔑 |
| `maxReplyAvatars` | number | ui | reply‑avatar cap (default 5) | — | |
...
| `annotations` / `unresolvedAnnotationsCount` / `unreadCount` | array/number | data | page totals | — | |
| `commentPinSelected` / `commentBubbleTargetPinHover` / `openDialog` | bool | ui | selected / hover / open | `velt-class="'hover': {commentBubbleTargetPinHover}"` | 🔑 |
...
| `commentCountType` / `variant` | string | ui | `'total'`/`'unread'` / variant | — | |
| `globalConfigSignal.featureState.customStatusesShown` / `.groupMatchedComments` / `.resolvedCommentsOnDom` / `.readOnly` | bool | feature | global feature flags | — | 🔑 |
...
| `totalComments` | number | data | comments on document | — | |
...
| `MIN_ALLOWED_WORDS_COUNT` / `MIN_`/`MAX_ALLOWED_CHARACTERS_COUNT` | number | ui | selection thresholds | — | |
| `darkMode` / `variant` / `parentLocalUIState.shadowDom` | bool/string | ui | theme / variant / shadow | — | 🔑 |
...
| `isResolvedCommentsOnDomFilterSelected` / `resolvedCommentsOnDom` | bool | ui | resolved filter | — | 🔑 |
...
| `multiThreadAnnotationId` / `multiThreadCommentAnnotation` / `nonDraftCommentsCount` | string/object/number | data | thread data | — | |
...
| `darkMode` / `dialogVariant` / `variant` / `uiState.shadowDom` | bool/string | ui | theme / variants / shadow | — | 🔑 |
...
| `componentConfig.contactsWithoutGroup` / `.groups` | array | data | ungrouped contacts / groups | — | |
...
| `componentConfig.customAutocompleteSearch` / `.expandMentionGroups` / `.showMentionGroupsFirst` / `.showMentionGroupsOnly` / `.customGroupsEnabled` | bool | ui | search / group modes | — | 🔑 |
...
| `componentConfig.moreFiltersVisible` | bool | ui | filter panel open | — | 🔑 |
...
| `componentConfig.virtualScrollData` / `.commentAnnotationsCountByFilters` / `.searchQuery` / `.selectedFilterTags` | array/object/string | data | data | — | |
...
| `notificationsByUserMap` / `notificationsByDocumentId` / `notificationsByDate` | object/array | data | grouped feeds | — | |
| `unreadNotificationsForYou` / `currentDocumentName` | number/string | data | unread badge / doc name | — | |
| `settingsConfig` / `settingsSelectedOption` | array/object | data | settings tree / selection | — | |
...
| `panelOpenMode` / `settingsLayout` / `pageSize` | string/number | ui | layout | — | |
...
| *(Tool)* `considerAllNotifications` / `panelShadowDom` | bool | feature/ui | count all / panel shadow | — | 🔑 |
...
| `componentConfig.skeletonLoading` / `.darkMode` / `.shadowDom` / `.variant` / `.excludeReactionIds` | bool/string/array | ui | loading/theme/shadow/exclusions | — | 🔑 |
...
| `componentConfig.user` | object | data | local user (hover user in tooltip) | — | |
| `componentConfig.maxUsers` | number | ui | avatar cap (default 5) | — | |
| `componentConfig.isActive` *(tooltip)* / `.lastActiveAt` | bool/number | app/data | active / last active | — | 🔑 |
| `componentConfig.shadowDom` / `.variant` | bool/string | ui | shadow / variant | — | 🔑 |
...
| `componentConfig.attendee` / `.isLocal` / `.color` / `.gainVolume` | object/bool/string/number | data/ui | per tile | — | 🔑(isLocal) |
...
| `recordingType` / `types` / `recorderModes` | string/array/object | data | active type / permitted / id map | — | |
...
| `totalTimeValue` / `currentTimeValue` / `elapsedTime` | number | data/state | durations | — | |
...
| `buttonLabel` / `darkMode` / `disabled` / `variant` / `shadowDom` | string/bool | ui | label / theme / state | — | 🔑(bools) |
...
| `highlightedTextIndex` | number | state | active segment | — | |
...
| `defaultVisibleCount` / `variant` | number/string | ui | per‑group count (5) / variant | — | |
...
**No wireframe variables / slots.** Customize via the `[template]` input on `velt-tag-pin` / `velt-tag-dialog`, CSS, or the headless `useTagAnnotations` / `useTagUtils` hooks. See [`../features/annotations-tags-arrows-areas.md`](../features/annotations-tags-arrows-areas.md).
