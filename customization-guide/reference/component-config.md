# Reference · Component config & layout props
...
> Use these with [primitives](../approaches/primitives.md) (and they apply to the same components in any framework as kebab‑case attributes). For trimming features on/off (`reactions`, `status`, …) see [`../approaches/primitives.md`](../approaches/primitives.md).
...
| `variant` | `string` | Selects which **wireframe variant** renders (register variants with `variant="…"` on the `Velt…Wireframe`; see [wireframes §3c](../approaches/wireframes.md)) |
...
| `variant` / `fullExpanded` / `darkMode` / `shadowDom` | — | Style / layout / theme / shadow‑DOM |

> **All `<VeltComments>` props** (the full 129‑prop table with types, defaults, and descriptions) are in [`props.md`](./props.md). This page focuses on the **layout/mode** props for the sidebar, dialog, and notifications.
