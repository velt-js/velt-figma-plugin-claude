...
- **Fix B (request interception) — SHIPPED, and the one to reach for first:** `scripts/_egress-relay.mjs`. Export `VELT_EGRESS_RELAY=1` and every browser-driving script (`verify-app`, `measure-block`/`openPage` and everything built on it, `capture-block`, `judge2-chromatic`, `judge2-chrome-probes`, `dom-snapshot`, `console-health`, `structural-invariants`, `interaction-state-probe`, `icon-live-lint`, `composed-audit`, `behaviour-check`, `discover-open`, `trials`, `run-tab`) intercepts each cross-origin request before Chromium's network stack sends it, performs it with Node's `fetch` (which does reach the sanctioned proxy), and fulfills the page with the result. Nothing disables TLS verification — Node still validates the chain and the proxy still enforces egress policy. Off unless the env var is set, so ordinary machines keep the browser's own (faster) networking. Needs `playwright-core` resolvable — set `$PLAYWRIGHT_CORE` if it isn't. Requires Fix A's `certutil` only if you prefer Fix A; where the sandbox has no NSS db (no `certutil`), Fix A is unavailable and this is the only option.
...
Neither fix repairs the sandbox proxy itself — journal the workaround you used so a resume can rebuild it. Known residue with Fix B: Firestore's `Listen/channel` is a stream and `route.fulfill()` cannot stream, so it is capped and re-polled; WebSockets bypass request interception entirely (`firebaseio.com` presence/realtime stays down). Neither blocks measurement — the comments UI renders — but expect those console errors and do not chase them as build defects.
