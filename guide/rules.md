...
- [ ] No green result stands in for work not done: every gate that could have measured NOTHING is proven to have measured SOMETHING (non-zero step/element/row count), and every write you depend on was read back (R31).
- [ ] Every closed delta row is closed by a fix, not by satisfying a mis-bound assertion: each assertion's selector agrees with `plan-style.json` for that `specNodeId`, and no value from a `brokenOverride`/`recoveredFrom` node was painted to (R32).

**R31 — A green result must never stand in for work that was not done: any operation that can produce an empty or absent result MUST fail loud.**
The dominant failure of the privado run was not a wrong value, it was five independent operations across unrelated scripts reporting success while persisting or verifying **nothing** — a smoke suite that ran zero steps and printed a tick (`[].every() === true`), a planner whose output was never written, an oversized journal payload silently dropped, a `report-block account` that changed nothing and exited clean, and a GAP disposition that hid every unrelated defect in its block. **Four of the five were caught only by reading state back off disk, not by an exit code.** So: a script that measured nothing exits non-zero with a distinct code (the precedent is `judge2-chrome-probes`' `exit 3` — "measured NOTHING is not a pass"); a lint that sweeps for unfilled placeholders ALSO validates the shape the consumer actually reads, because a restructured artifact has no placeholders left and sails through; a claim in a report is DERIVED from the tally it describes, never templated; and after any write you depend on, **read it back**. When you cannot prove an operation did what it said, treat it as not done.

**R32 — A delta row disappearing is only evidence of a fix when the assertion is bound to the RIGHT element.**
A mis-bound assertion is not merely noisy — it *invites* a plausible fix that paints a literal onto the wrong element, and the delta metric then *rewards* it: the measurement improves while the design gets no closer. Two design nodes with coincident boxes can collapse onto one selector, so a wrapper's declarations end up asserted against its own child. Before treating a closed row as a fix, confirm the assertion's selector is the one `plan-style.json` maps that `specNodeId` to; where they disagree the assertion must be **re-bound, not satisfied**. Same discipline for a value you cannot bind at all: a node flagged `brokenOverride`/`recoveredFrom` was back-filled from the component master and describes that master, not the frame you are measuring — such a value is unfalsifiable against the live app and must never be painted to. The contrast that teaches it, from one pass: on the header border the live paint was checked against the plan first and the correct answer was to change **nothing**; on the card `align-items` it was not checked, and the "fix" was an inert rule that contradicted the design.
