...
| **Composer collapses, dialog previews first comment** | Composer opens collapsed (`composerMode: 'default'`); an unselected pin dialog shows only the first comment; selecting expands to all. | See the composer + collapse matrices in [behaviors index](../behaviors.md). |
...
| **`@`-mentions, reactions, attachments, status, resolve, seen-by ON; priority, visibility, screenshot OFF** | The on/off baseline. | See [`feature-flags.md`](../feature-flags.md). |
...
| `customReactions` | — | `setCustomReactions(ReactionMap)` — replaces the default emoji set with your map. | Shape in [`component-config.md`](../component-config.md). Requires `reactions` on. |
...
| `customStatus` | — | `setCustomStatuses(array)` — replaces default statuses. Each entry has `type:'default'\|'ongoing'\|'terminal'`; a `terminal` status marks the thread resolved/closed. | Use this for workflow states only — **not** arbitrary tags (use `customCategory` for categories). Requires `status` on. |
...
| `customPriority` | — | `customFilterService.setCustomPriorityFilters(array)` — replaces priority options. | Stored via the **custom-filter** service (so it also feeds sidebar priority filters). Requires `priority` on to surface in the composer. |
| `customCategory` | — | `customFilterService.setCustomCategories(array)` — defines selectable comment categories. | The correct tool for arbitrary tagging/grouping; pairs with `autoCategorize` and sidebar category filters. |
...
| `minimapPosition` | — | Writes `minimapPosition` into the component config signal + analytics config — corner placement of the minimap. | Only effective with `minimap` on. |
...
| `commentPinHighlighter` | — | `enableCommentPinHighlighter`/`disable` — highlights the target element when its pin is active/selected. | Visual association aid; no data effect. |
...
| `collapsedComments` | `false` | `enableCollapsedComments`/`disable` — keeps the thread collapsed (first + last only, "N more replies" divider between) even when selected. | Full interaction matrix with `collapsedRepliesPreview` and `showAllComments` in [behaviors index](../behaviors.md). |
| `collapsedRepliesPreview` | `false` | Renders the first + "N more replies" + last teaser **before** selection. | See the exact rendered-structure matrix in [behaviors index](../behaviors.md). |
...
| `atHereEnabled` | — | `contactService.enableAtHere`/`disable` — enables the `@here` broadcast mention. | `atHereLabel`/`atHereDescription` customize its row. |
| `atHereLabel` | — | `contactService.setAtHereLabel(str)` — label text for the `@here` entry (only set when truthy). | Effective only with `atHereEnabled`. |
| `atHereDescription` | — | `contactService.setAtHereDescription(str)` — description/subtext for the `@here` entry (only when truthy). | Pairs with `atHereLabel`. |
| `customListDataOnAnnotation` | — | `createCustomListDataOnAnnotation(data)` — injects a custom dropdown dataset scoped to the annotation level. | `CustomAnnotationDropdownData` shape; for per-annotation pickers. |
| `customListDataOnComment` | — | `autocompleteService.create(data)` — registers a custom autocomplete dataset at the comment level. | `AutocompleteData` shape; independent of `customAutocompleteSearch` (this seeds a named list, that swaps the mention backend). |
...
| `streamViewContainerId` | — | Plain string input — id of the element that hosts the stream view. | Only meaningful with `streamMode`. |
...
| `composerMode` | `'default'` | `setComposerMode(value)` — `'default'` opens collapsed (expands on focus); `'expanded'` renders full actions immediately. | See the composer matrix in [behaviors index](../behaviors.md). |
...
| Prop | Default | Behavior (runtime) | Interactions & gotchas |
|---|---|---|---|
| `commentPlaceholder` | — | `setCommentplaceholder(str)` — placeholder text for a new top-level comment composer. | Falls back to SDK default string when unset. |
| `replyPlaceholder` | — | `setReplyplaceholder(str)` — placeholder for the reply composer. | Independent of `commentPlaceholder`. |
| `editPlaceholder` | — | `setEditplaceholder(str)` — generic edit-mode placeholder. | `editCommentPlaceholder`/`editReplyPlaceholder` are the more specific overrides. |
| `editCommentPlaceholder` | — | `setEditcommentplaceholder(str)` — placeholder when editing a comment specifically. | More specific than `editPlaceholder`. |
| `editReplyPlaceholder` | — | `setEditreplyplaceholder(str)` — placeholder when editing a reply. | More specific than `editPlaceholder`. |
...
| `persistentCommentShadowDom` | — | Writes `persistentCommentShadowDOM` into config + analytics — isolates the persistent composer. | Set to `true` by master `shadowDom`; only relevant with `persistentCommentMode`. |
...
| `dialogDarkMode` | — | Dialog-level dark mode override. (unconfirmed — no dedicated container setter; the dialog surface reads container dark mode / its own `darkMode` prop on `<VeltCommentDialog>`.) | Prefer setting `darkMode` on the container, or `darkMode` directly on `<VeltCommentDialog>`. |
...
| `onCommentAdd` | A comment is added | **Legacy** — wired via `setCommentAddedEmitter`. Prefer the event hook `useAddCommentAnnotation`; see [`events.md`](../events.md). |
